 ...
 * @Description:
 */
@Api(tags = {SwaggerConfig.TQ_TAG_16})
@RestController
@RequestMapping("/subscribeApp")
public class SubscribeAppController {

    @Resource
    TqSubscribeAppConfigService service;

    @ApiOperation(value = "查询所有应用")
    @PostMapping("/getAll")
    public ResultEntity<Page<TqSubscribeAppConfigVO>> getAll(@RequestBody Page<TqSubscribeAppConfigVO> page) {
        return ResultEntityBuild.build(ResultEnum.SUCCESS, service.getAll(page));
    }

    @ApiOperation("添加应用")
    @PostMapping("/add")
    public ResultEntity<Object> addData(@RequestBody TqSubscribeAppConfigPO po) {
        return ResultEntityBuild.build(service.addData(po));
    }

    @ApiOperation("编辑应用")
    @PostMapping("/edit")
    public ResultEntity<Object> editData(@RequestBody TqSubscribeAppConfigPO po) {
