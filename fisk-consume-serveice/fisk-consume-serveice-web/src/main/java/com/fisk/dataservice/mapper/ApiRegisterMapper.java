...
public interface ApiRegisterMapper extends FKBaseMapper<ApiConfigPO> {
    ...
    @Insert("INSERT INTO `tb_api_config`(`api_name`, `api_code`, `api_desc`, `api_type`, `datasource_id`, `table_name`, `table_type`, `table_business_type`, `create_sql`, `create_time`, `create_user`,`del_flag`) VALUES (#{apiName}, #{apiCode}, #{apiDesc}, #{apiType}, #{datasourceId}, #{tableName}, #{tableType}, #{tableBusinessType}, #{createSql}, #{createTime}, #{createUser},1);")
     ...
     * @return 查询结果
     */
    @Select("SELECT id,api_code,api_name,api_code,api_type,datasource_id,table_name,table_type,table_business_type,create_sql FROM tb_api_config WHERE api_code=#{apiCode} AND del_flag=1;")
