        ...
        if (data.specialType != SpecialTypeEnum.NONE.getValue()){
            ...
            switch (SpecialTypeEnum.getEnum(data.specialType)){
                case KSF_NOTICE:
                    ...
                    data.setSqlScript("SELECT \n" +
                            "\ttstbtime2key, \n" +
                            "\tid, \n" +
                            "\tmuid, \n" +
                            "\tname, \n" +
                            "\tsex\n" +
                            "FROM dbo.fact_tstbtime2");
                    break;
                case KSF_ITEM_DATA:
                ...
                case KSF_INVENTORY_STATUS_CHANGES:
                    ...
                    data.setSqlScript("select * from ods_sap_ksf_inventory_sys where TO_TIMESTAMP(fi_createtime, 'YYYY-MM-DD HH24:MI:SS.US') > '${startTime}'::TIMESTAMP AND TO_TIMESTAMP(fi_createtime, 'YYYY-MM-DD HH24:MI:SS.US') <= '${endTime}'::TIMESTAMP; select * from ods_sap_ksf_inventory WHERE fidata_batch_code in (select fidata_batch_code from ods_sap_ksf_inventory_sys where TO_TIMESTAMP(fi_createtime, 'YYYY-MM-DD HH24:MI:SS.US') > '${startTime}'::TIMESTAMP AND TO_TIMESTAMP(fi_createtime, 'YYYY-MM-DD HH24:MI:SS.US') <= '${endTime}'::TIMESTAMP);");
                    break;
            }
        }
        if (!this.save(data)) {
