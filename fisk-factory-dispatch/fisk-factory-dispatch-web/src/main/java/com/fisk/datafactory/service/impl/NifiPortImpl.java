...
public class NifiPortImpl implements INifiPort {
    ...
    public ResultEntity<NifiPortsDTO> getFilterData(PortRequestParamDTO dto) {
        ...
        switch (dto.flag) {
            // pid == 0
            case 1:
                List<NifiCustomWorkflowDetailPO> list1 = nifiCustomWorkflowDetailImpl.query().eq("workflow_id", dto.id).eq("pid", 0).ne("component_type","触发器").list();
            ...
            // pid != 0
            case 2:
                List<NifiCustomWorkflowDetailPO> list2 = nifiCustomWorkflowDetailImpl.query().eq("pid", dto.pid).ne("component_type","触发器").list();
