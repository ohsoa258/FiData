...
public class DatacheckStandardsGroupServiceImpl extends ServiceImpl<DatacheckStandardsGroupMapper, DatacheckStandardsGroupPO> implements IDatacheckStandardsGroupService {
     ...
     */
    @Override
    public List<DatacheckStandardsGroupVO> getDataCheckStandardsGroup(Integer standardsId) {
        ...
        if (!CollectionUtils.isEmpty(standardByMenuId)) {
            ...
            // 将PO对象列表转换为VO对象列表
            groupDtoList = groupPOList.stream().map(DatacheckStandardsGroupMap.INSTANCES::poToVo).collect(Collectors.toList());
            // 根据分组ID列表获取所有规则信息
