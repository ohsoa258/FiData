...
public interface BusinessFilterMapper extends FKBaseMapper<BusinessFilterPO> {
    ...
    @Insert("INSERT INTO `tb_bizfilter_module`(`template_id`, `datasource_id`, `datasource_type`, `module_name`, `filter_step`, `table_name`, `pro_table_name`, `module_rule`, `run_time_cron`, `module_exec_sort`, `module_state`, `create_time`, `create_user`, `update_time`, `update_user`, `del_flag`) VALUES (#{templateId}, #{datasourceId}, #{datasourceType}, #{moduleName}, #{filterStep}, #{tableName}, #{proTableName}, #{moduleRule}, #{runTimeCron}, #{moduleExecSort}, #{moduleState}, #{createTime}, #{createUser},1);")
